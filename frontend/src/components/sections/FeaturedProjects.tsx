"use client";
import Link from "next/link";
import Image from "next/image";
import { motion } from "framer-motion";
import { useInView } from "react-intersection-observer";
import { ArrowRight } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { projectsApi, getImageUrl } from "@/lib/api";
import { getCategoryLabel } from "@/lib/utils";

export function FeaturedProjects() {
  const { ref, inView } = useInView({ triggerOnce: true, threshold: 0.1 });
  const { data: projects = [], isLoading } = useQuery({
    queryKey: ["featured-projects"],
    queryFn: projectsApi.featured,
  });

  return (
    <section ref={ref} className="section-padding bg-page" aria-labelledby="featured-heading">
      <div className="max-w-7xl mx-auto container-padding">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-20">
          <div className="max-w-xl">
            <motion.div
              className="flex items-center gap-3 mb-5"
              initial={{ opacity: 0, x: -20 }}
              animate={inView ? { opacity: 1, x: 0 } : {}}
              transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            >
              <span className="gold-line" />
              <span className="section-label">Selected Work</span>
            </motion.div>
            <motion.h2
              id="featured-heading"
              className="text-section font-serif text-[var(--text-primary)]"
              initial={{ opacity: 0, y: 30 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.8, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
            >
              Featured projects
            </motion.h2>
          </div>
          <motion.div
            initial={{ opacity: 0 }}
            animate={inView ? { opacity: 1 } : {}}
            transition={{ delay: 0.3, duration: 0.6 }}
          >
            <Link
              href="/portfolio"
              className="inline-flex items-center gap-2 text-[11px] tracking-[0.2em] uppercase text-[var(--text-muted)] hover:text-gold-500 transition-colors duration-500 group"
            >
              All Projects
              <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform duration-500" />
            </Link>
          </motion.div>
        </div>

        {/* Grid */}
        {isLoading ? (
          <ProjectsSkeleton />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {projects.slice(0, 4).map((project, i) => (
              <motion.article
                key={project.id}
                initial={{ opacity: 0, y: 40 }}
                animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.8, delay: i * 0.12, ease: [0.16, 1, 0.3, 1] }}
                className={i === 0 ? "md:col-span-2" : ""}
              >
                <Link href={`/portfolio/${project.slug}`} className="group block relative overflow-hidden">
                  <div className={`relative overflow-hidden ${i === 0 ? "h-[70vh] max-h-[700px]" : "h-80 md:h-96"}`}>
                    <Image
                      src={getImageUrl(project.cover_image)}
                      alt={project.title}
                      fill
                      className="object-cover transition-transform duration-[1.2s] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-105"
                      sizes={i === 0 ? "100vw" : "(max-width: 768px) 100vw, 50vw"}
                      loading={i < 2 ? "eager" : "lazy"}
                    />
                    <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors duration-700" />

                    {/* Category */}
                    <div className="absolute top-6 left-6">
                      <span className="text-[9px] tracking-[0.25em] uppercase text-white/70">
                        {getCategoryLabel(project.category)}
                      </span>
                    </div>

                    {/* Info overlay - bottom */}
                    <div className="absolute bottom-0 left-0 right-0 p-8">
                      <h3 className="font-serif text-2xl md:text-3xl text-white mb-2" style={{ fontWeight: 400, letterSpacing: "-0.02em" }}>
                        {project.title}
                      </h3>
                      <div className="flex items-center gap-4 text-[11px] tracking-[0.1em] text-white/50">
                        <span>{project.location}</span>
                        <span className="w-1 h-1 rounded-full bg-white/30" />
                        <span>{project.year}</span>
                      </div>

                      <div className="mt-4 overflow-hidden h-0 group-hover:h-6 transition-all duration-500">
                        <span className="flex items-center gap-2 text-[10px] tracking-[0.2em] uppercase text-gold-400">
                          View Project <ArrowRight size={11} />
                        </span>
                      </div>
                    </div>
                  </div>
                </Link>
              </motion.article>
            ))}
          </div>
        )}
      </div>
    </section>
  );
}

function ProjectsSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className={`animate-pulse ${i === 0 ? "md:col-span-2" : ""}`}>
          <div className={`bg-[var(--border)] ${i === 0 ? "h-[70vh] max-h-[700px]" : "h-80 md:h-96"}`} />
        </div>
      ))}
    </div>
  );
}
